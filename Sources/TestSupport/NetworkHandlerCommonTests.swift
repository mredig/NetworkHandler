...
public struct NetworkHandlerCommonTests<Engine: NetworkEngine>: Sendable {
	...
	public func uploadStreamProvider(
		engine: Engine,
		file: String = #fileID,
		filePath: String = #filePath,
		line: Int = #line,
		...
		let streamProvider = UploadEngineRequest.StreamProvider(
			block: { startOffset, requestedByteCount in
				let endIndex = min(startOffset + requestedByteCount, randomData.endIndex)
				let range = startOffset..<endIndex
				guard range.isOccupied else { return nil }
				return randomData[range]
			},
			hasDataAvailableBlock: { startOffset in
				let hasData = (startOffset < randomData.endIndex)
				return hasData
			},
			totalStreamBytes: randomData.count)
		...
		let atomicRequest = AtomicValue(value: NetworkRequest.upload(signedRequest, payload: .streamProvider(streamProvider)))
		let delegate = await Delegate(onRequestModified: { del, orig, new in
			atomicRequest.value = new
		})

		_ = try await nh.uploadMahDatas(for: signedRequest, payload: .streamProvider(streamProvider), delegate: delegate)
		#expect(
